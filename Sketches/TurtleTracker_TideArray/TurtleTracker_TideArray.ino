...
float maxEPE = 30.0;   // Maximum EPE value that will be used to keep GPS coordinates
uint32_t myAcqTime =  30;  // Seconds to acquire a GPS fix
uint32_t myPeriodHT  = 600;   // Repeat period in seconds for GPS fixes during high tide
uint32_t myPeriodLT  = 1800; // Repeat period in seconds for GPS fixes during low tide
...
// date and time to inititialize RTC clock
uint16_t highTideRefYear = 2020;
uint16_t highTideRefMonth = 1; // January = 1 and December = 12
uint16_t highTideRefDay = 1;
uint16_t highTideRefHour = 0;  // 24 hour clock
uint16_t highTideRefMinute = 20;
uint16_t highTideRefSecond = 0;

uint32_t refHighTide;  // Number of second from beginning of year to reference high tide 
uint32_t millisNextHighTide;
int32_t secBeforeHTStart; // Seconds befor starting high tide cycles
     ...
     if (myLocation.fixType() != GNSSLocation::TYPE_TIME)
      ...
      if( (myLocation.fixType() != GNSSLocation::TYPE_2D) && (EPE <= maxEPE) && myLocation.fullyResolved()) 
      {
          ...
          if (!initialized) 
          {
              ...
              Serial.println("secBeforeHTStart init");
              Serial.println(secBeforeHTStart);
